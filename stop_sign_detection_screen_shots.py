...
import os
import logging
# import serial  # Commented out as no Arduino is available
...
def prompt_for_screenshot():
    """Prompt user to prepare a stop sign screenshot."""
    print("No physical stop sign detected. Please follow these steps:")
    print("1. Find an image of a stop sign online (e.g., Google Images).")
    print("2. Take a screenshot of the stop sign image.")
    print("3. Display the screenshot on your screen (e.g., open in an image viewer).")
    print("4. Point your webcam at the screenshot during testing.")
...
def main():
    ...
    # Commented out serial communication for testing without Arduino
    """
    try:
    ...
    video = cv2.VideoWriter('stop_sign_videos/out/detected_output_screenshot.avi', fourcc, 20, (width, height))
    ...
    while cap.isOpened():
        ...
        # Handle stop sign detection and testing feedback
        ...
        if (len(stop_signs) > 0 or fake_stop_triggered) and not send_stop:
            ...
            # Simulate sending '1' with print and on-screen text
            status = "STOP DETECTED (FAKE)" if fake_stop_triggered else "STOP DETECTED"
            print(status)
            logging.info(status)
            # ser.write(b'1')  # Commented out
            ...
            if send_stop and (current_time - last_detection_time >= 3):
                print("MOVING")
                logging.info("MOVING")
                # ser.write(b'0')  # Commented out
                cv2.putText(frame, "NO STOP SIGN", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                send_stop = False
            # If no stop sign or after 3 seconds, show moving
            elif not send_stop:
                print("MOVING")
